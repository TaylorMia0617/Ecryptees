...
当前 Android 版本：`1.0.13`（versionCode 14）。
...
1. 安装 `dist/Ecryptees-v1.0.13.apk`；从旧版本升级时直接覆盖安装，不要先卸载。
...
也可以在“漫画 → 网页链接”中粘贴 HTTPS 页面地址。点击“分析网页”才会联网；分析结果按页面顺序列出，用户确认、删除或调整顺序后才加入漫画。应用会先使用静态 HTML；如果静态页面没有图片，Android APK 才会创建隔离的临时 WebView，运行目标页面脚本、滚动触发懒加载并分块复制最终图片，随后销毁临时页面及其 Cookie、缓存和站点存储。该环境不读取 Ecryptees 主 WebView 状态，但目标页面脚本本身仍会向其网站及页面引用的第三方地址发起请求。
...
- 网页导入优先解析静态 HTML；仅当静态页面没有图片时，Android 才在无既有 Cookie 和登录状态的临时 WebView 中运行页面脚本，并在完成后清除临时站点数据。浏览器/PWA 版本仍受目标网站 CORS 策略限制。
...
未来计划在不破坏现有离线体验的前提下，增加一个由用户主动进入的“通道模式”。当前 `1.0.13` 尚未包含通道协议、项目服务器、哑存储或打洞直连；现有网络能力只用于用户主动触发的网页导入。

```text
App
├─ 离线模式
│  └─ 现有 .ecomic 阅读器 / 打包器；零网络，保持现有格式与书架行为
└─ 通道模式
   ├─ 哑存储模式：服务器长期在线，只保存密文 Blob，不持有解密密钥
   └─ 打洞直连模式：不长期保存内容，发送双方必须至少两人同时在线
```

计划坚持以下边界：

- 现版本只有用户明确点击网页导入的“分析网页”时才创建连接；未来通道也必须由用户主动切入。
- 离线模式不会自动探测服务器、后台同步或上传任何内容。
- 哑存储服务器只接触密文和必要的传输元数据，密钥只保留在通信端。
- 打洞直连优先端到端传输；如果网络环境无法建立连接，应明确失败，不静默转存明文或降低认证强度。
- 新通道协议必须独立版本化，不能改变现有 `.ecomic` v1 文件内容或旧书架兼容性。

需要注意：Android 的 `INTERNET` 是安装时声明的普通权限，不会在首次联网时弹出运行时授权框。自 1.0.11 起，网页导入已声明该权限，因此这里保证的是“默认不联网、必须由用户点击触发”，而不是“APK 没有网络权限”。未来通道模式仍需要单独设计连接、密钥和隐私边界。
...
构建脚本会生成版本化 APK，例如 `dist/Ecryptees-v1.0.13.apk`，并同步覆盖稳定文件名 `dist/Ecryptees.apk`。脚本会验证 APK 内部版本和发布证书；覆盖安装必须继续使用同一个 applicationId 与签名证书。
