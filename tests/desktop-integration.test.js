...
test('release uses the GUI subsystem and desktop web imports use a bounded native bridge', () => {
    ...
    const adapter = read('js/network-adapter.js');
    ...
    assert.match(rust, /enum AddressClass/);
    assert.match(rust, /ClashFakeIp/);
    assert.match(rust, /不能直接访问 Clash Fake-IP/);
    assert.match(rust, /MAX_DIAGNOSTIC_ENTRIES/);
    assert.match(rust, /format_route_diagnostic/);
    ...
    assert.match(comic, /const nativeNetwork = networkAdapter/);
    assert.match(adapter, /platform === 'android'[\s\S]*network\.beginRenderedPageCapture\(String\(url \|\| ''\), limit\)/);
    assert.match(adapter, /network\.beginRenderedPageCapture\([\s\S]*interactiveVerification === true/);
...
test('Windows dynamic capture is isolated, token scoped, and blocks private resources', () => {
    ...
    assert.match(rust, /classify_network_url/);
    ...
    assert.match(rust, /checkingChallenge/);
    assert.match(rust, /web_resource_context_label/);
    assert.match(rust, /Ecryptees 网页验证/);
    assert.match(rust, /Duration::from_secs\(8\)/);
    assert.match(rust, /--no-proxy-server/);
    ...
    assert.match(bridge, /getNetworkMode/);
    assert.match(bridge, /read_desktop_rendered_page_image_chunk/);
    assert.match(comic, /const renderedNetwork = networkAdapter/);
...
test('desktop settings expose three separate locations and migration choices', () => {
    ...
    assert.match(html, /id="desktopNetworkModeSelect"/);
    assert.match(html, /id="webImportDiagnosticsPanel"/);
    assert.match(settings, /initializeDesktopNetworkSetting/);
    assert.match(read('js/comic-app.js'), /updateWebImportDiagnostics/);
